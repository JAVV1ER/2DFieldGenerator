...
{
    public static float[] GenerateNoiseMap(int width, int height, int seed, float scale, int octaves, Vector2 offset, float frequency, float amplitude)
    {
        ...
        for (int y = 0; y < height; y++)
        {
            for (int x = 0; x < width; x++)
            {
                ...
                for (int i = 0; i < octaves; i++)
                {
                    ...
                    // Расчёт амплитуды, частоты и компенсации для следующей октавы(пока убрано)
